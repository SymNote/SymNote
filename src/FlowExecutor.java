        ...
        switch (functionName) {
            ...
            default:
                if (interpreter.routines.containsKey(functionName)) {
                    ...
                    try {
                        int expectedParams = routineCtx.parameters() == null ? 0 : routineCtx.parameters().param().size();
                        if (args.size() != expectedParams) {
                            throw new RuntimeException("Routine '" + functionName + "' expected " + expectedParams + " arguments but got " + args.size() + " at line " + ctx.getStart().getLine());
                        }
                        ...
                        if (routineCtx.parameters() != null) {
                            for (int i = 0; i < routineCtx.parameters().param().size(); i++) {
                                ...
                                interpreter.checkType(paramType, argValue, "argument '" + paramName + "' in routine '" + functionName + "'", ctx.getStart().getLine());
                        ...
                        if (expectedType.equals("void")) {
                            if (returnValue != null) {
                                throw new RuntimeException("Void routine '" + functionName + "' cannot return a value at line " + line);
                            }
                            return null;
                        }
                        ...
                        if (returnValue == null) {
                            throw new RuntimeException("Routine '" + functionName + "' must return a value of type '" + expectedType + "' at line " + line);
                        }
                ...
                SymNoteParser.TrackDeclContext trackCtx = (SymNoteParser.TrackDeclContext) interpreter.env.getTrack(functionName);
                if (trackCtx == null) {
                    throw new RuntimeException("Undefined function or track '" + functionName + "' at line " + ctx.getStart().getLine());
                }
                ...
                try {
                    ...
                    if (args.size() != expectedParams) {
                        throw new RuntimeException("Track '" + functionName + "' expected " + expectedParams + " argument(s) but got " + args.size() + " at line " + ctx.getStart().getLine());
                    }
                    ...
                    if (trackCtx.parameters() != null) {
                        for (int i = 0; i < trackCtx.parameters().param().size(); i++) {
                            ...
                            interpreter.checkType(paramType, argValue, "argument '" + paramName + "' in track '" + functionName + "'", ctx.getStart().getLine());
