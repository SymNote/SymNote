...
public class DocumentationTest {
    ...
    class VariableTypes {
        ...
        void int_rejects_float() {
            ...
            assertFalse(r.isSuccess());
            assertTrue(r.error.getMessage().contains("mismatch"), "Got: " + r.error.getMessage());
        }
    ...
    class ControlFlow {
        ...
        void if_requires_bool() {
            ...
            assertFalse(r.isSuccess());
            assertTrue(r.error.getMessage().contains("bool"), "Got: " + r.error.getMessage());
        }
        ...
        void if_no_brace_scope_leak() {
            TestHelper.Result r = TestHelper.run(
                    "if (true)\n" +
                            ...
                            "print(hidden);");
            assertFalse(r.isSuccess());
            assertTrue(r.error.getMessage().contains("hidden"), "Got: " + r.error.getMessage());
        }
        ...
        void inner_scope_destroyed() {
            TestHelper.Result r = TestHelper.run(
                    "if (true) { int y = 20; }\n" +
                            "print(y);");
            assertFalse(r.isSuccess());
            assertTrue(r.error.getMessage().contains("y") && !r.isSuccess(), "Got: " + r.error.getMessage());
        }
        ...
        void lexical_scope_isolation() {
            TestHelper.Result r = TestHelper.run(
                    "routine inner() returns int { return secret; }\n" +
                            ...
                            "print(outer());");
            assertFalse(r.isSuccess());
            assertTrue(r.error.getMessage().contains("secret"), "Got: " + r.error.getMessage());
        }
    ...
    // ─────────────────────────────────────────────
    // diagnostics.md — "Advanced Diagnostics in SymNote"
    // ─────────────────────────────────────────────
    @Nested
    @DisplayName("diagnostics.md")
    class DiagnosticsDoc {

        @Test
        @DisplayName("1. Did You Mean? Suggestions")
        void d1_did_you_mean() {
            TestHelper.Result r = TestHelper.run(
                    "int volume = 80;\n" +
                            "print(volum);");
            assertFalse(r.isSuccess());
            assertTrue(errorMsg(r).contains("volume"), "Should suggest 'volume'");
        }

        @Test
        @DisplayName("2. Call Stack Traces")
        void d2_call_stack_traces() {
            TestHelper.Result r = TestHelper.run(
            ...
            assertFalse(r.isSuccess());
            assertTrue(
                    errorMsg(r).contains("divide") && errorMsg(r).contains("process") && errorMsg(r).contains("zero"),
                    "Should include call stack and zero division error");
        }

        @Test
        @DisplayName("3. Rich Type Mismatches")
        void d3_rich_type_mismatches() {
            TestHelper.Result r = TestHelper.run(
                    "int speed = 3.14;");
            assertFalse(r.isSuccess());
            assertTrue(errorMsg(r).contains("int") && errorMsg(r).contains("float") && errorMsg(r).contains("3.14"),
                    "Should include rich type mismatch details");
        }

        @Test
        @DisplayName("4. Built-in Function Validation (Invalid)")
        void d4_builtin_validation_invalid() {
            TestHelper.Result r = TestHelper.run(
                    "track PianoTrack() {\n" +
                            "    use_synth(\"piano\");\n" +
                            "}\n" +
                            "parallel { PianoTrack(); }");
            assertFalse(r.isSuccess());
            assertTrue(errorMsg(r).contains("use_synth") && errorMsg(r).contains("string"),
                    "Should mention use_synth and string type error");
        }

        @Test
        @DisplayName("4. Built-in Function Validation (Valid)")
        void d4_builtin_validation_valid() {
            TestHelper.Result r = TestHelper.run(
        ...
        @Test
        @DisplayName("5. Grid Note Resolution Validation")
        void d5_grid_note_validation() {
            TestHelper.Result r = TestHelper.run(
                    "string myNote = \"C4\";\n" +
                            "synth myPiano = load_synth(\"piano\");\n" +
                            "track MyTrack() {\n" +
                            ...
                            "parallel { MyTrack(); }");
            assertFalse(r.isSuccess());
            assertTrue(errorMsg(r).contains("myNote") && errorMsg(r).contains("string") && errorMsg(r).contains("note"),
                    "Should point out the invalid note variable in grid");
        }
    }
    ...
    class LevelArchitecture {
        ...
        void l2_local_var_isolation() {
            ...
            assertTrue(r.error.getMessage().contains("local"), "Got: " + r.error.getMessage());
        }
